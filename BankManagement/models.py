...
class Department(models.Model):
    ...
    Department_Manger_ID = models.CharField('Department_Manger_ID', max_length=MAX_CHAR_LEN, blank=False)
    Department_Name = models.CharField('Department_Name', max_length=MAX_CHAR_LEN, blank=False)
    Department_Type = models.CharField('Department_Type', max_length=MAX_CHAR_LEN,blank=False)
...
class Customer(models.Model):
    ...
    Contact_Person_Phone_Number = models.DecimalField('Contact_Person_Name', max_digits=11, decimal_places=0,
                                                      blank=False)
...
class CustomerToCA(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['Customer_ID', 'CAccount_Open_Bank_Name'],
                                    name='One customer is only allowed to open one CA in one bank'),
...
class CustomerToSA(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['Customer_ID', 'SAccount_Open_Bank_Name'],
                                    name='One customer is only allowed to open one SA in one bank'),
...
class Loan(models.Model):
    ...
    Loan_Status = models.CharField ('Loan_Status', max_length=1)
    Loan_Bank_Name = models.ForeignKey(Bank, on_delete=models.CASCADE)
